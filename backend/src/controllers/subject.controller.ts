...
// Helper to calculate skip / attend details
export const calculateAttendanceStats = (attended: number, total: number, requiredPercent: number) => {
  ...
  if (currentPercent >= requiredPercent) {
    ...
    // A / (T + S) >= reqFraction => S <= A/reqFraction - T
    if (reqFraction > 0) {
      classesCanSkip = Math.floor(attended / reqFraction - total);
      if (classesCanSkip < 0) classesCanSkip = 0;
    }
  } else {
    // How many classes to attend consecutively
    // (A + N) / (T + N) >= reqFraction => N >= (reqFraction*T - A) / (1 - reqFraction)
    if (reqFraction < 1) {
      classesNeeded = Math.ceil((reqFraction * total - attended) / (1 - reqFraction));
    } else {
      classesNeeded = 999; // impossible to reach 100% if already missed
    }
  }

  // Safe statuses: Green (Safe), Yellow (Warning - within 3% above limit), Red (Critical - below limit)
...
export const deleteSubject = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    // Clean up related logs and timetable
    await AttendanceLog.deleteMany({ subjectId: req.params.id });
    const Timetable = require('../models/timetable.model').default;
